...
for (int jack_index=0;jack_index<Njack;jack_index++){	
	...
	//compute yXXy
	  ...
	  if(pass_num==1)
	  	yXXy.block(bin_index*phenocount,0,phenocount,1)+=temp_yxxy.transpose();
	  //else  
...
for (int rep_index=0 ; rep_index<phenocount; rep_index++){
	...
	for (int i=0;i<Nbin;i++){
		...
		if(both_side_cov==true){
                ...
                tk_res=C2.sum();
                tk_res=tk_res/len[i]/Nz; 
                 b_trk(i,0)=mask.col(rep_index).sum()-tk_res; 
       		}
	}	

	if(both_side_cov==true)
	   NC=mask.col(rep_index).sum()-cov_num;
	else
	   NC=mask.col(rep_index).sum();	

	double yy=all_yy(0,rep_index);
	X_l<<A_trs,b_trk,b_trk.transpose(),NC;
	Y_r<<c_yky,yy;
