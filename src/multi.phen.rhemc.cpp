...
MatrixXdr  compute_yXXy(int num_snp){

	MatrixXdr pheno_sum=pheno.colwise().sum();
        MatrixXdr res(num_snp, phenocount);

	
	if(use_mailman==true)
		multiply_y_pre_fast(pheno,phenocount,res,false);
	else
		 res=gen*pheno;
...
MatrixXdr  compute_yVXXVy(int num_snp){
        ...
        if(use_mailman==true)
                multiply_y_pre_fast(new_pheno,phenocount,res,false);
        else
                 res=gen*new_pheno;
...
yXXy=MatrixXdr::Zero(phenocount*Nbin,Njack+1);
...
for (int k=0;k<phenocount;k++){
 for (int i=0;i<Nbin;i++){
        ...
        b_trk(i,0)=mask.col(k).sum()-tk_res;
        }
 }
...
MatrixXdr se_h2_est=jack_se(jack);

//cout<<se_h2_est<<endl;
//cout<<se_point_est<<endl;
...
outfile<<"sigma^2_"<<i<<" ";
cout<<"sigma^2_"<<i<<" ";
outfile<<"se_sigma^2_"<<i<<" ";
cout<<"se_sigma^2_"<<i<<" ";
}
outfile<<"sigam^2_e"<<" ";
cout<<"sigam^2_e"<<" ";
outfile<<"se_sigam^2_e"<<" ";
cout<<"se_sigam^2_e"<<" ";
for(int i=0;i<Nbin;i++){
outfile<<"h^2_"<<i<<" ";
cout<<"h^2_"<<i<<" ";
...
for (int phen_index=0;phen_index<phenocount;phen_index++){
    ...
    outfile<<phen_index<<" ";
    for(int i=0;i<(Nbin+1);i++){
     	cout<<point_est(i,phen_index)<<" ";
        outfile<<point_est(i,phen_index)<<" ";
	cout<<se_point_est(((Nbin+1)*phen_index)+i,0)<<" ";
        outfile<<se_point_est(((Nbin+1)*phen_index)+i,0)<<" ";  
    }
    for(int i=0;i<(Nbin+1);i++){
        cout<<all_h2(i,phen_index)<<" ";
        outfile<<all_h2(i,phen_index)<<" ";
	cout<<se_h2_est(((Nbin+1)*phen_index)+i,0)<<" ";
        outfile<<se_h2_est(((Nbin+1)*phen_index)+i,0)<<" ";
    }
    cout<<endl;
    outfile<<endl;
